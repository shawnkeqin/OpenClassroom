...
<seminarCardRequest  v-for="seminar in seminar"
      :key="seminar.id"
      :seminar="seminar"
      class="seminar-item" >
  </seminarCardRequest >
 ...
 <!--  <a-range-picker
    ...
    <a-select-option v-for="i in 25" :value="(i + 9).toString(36) + i" :key="(i + 9).toString(36) + i">
      {{ (i + 9).toString(36) + i }}
    </a-select-option>
  </a-select>
  </a-form-item>
     <p align="center">Tags</p>
       <a-select v-model="tags" mode="tags" style="width: 100%" placeholder="Tags Mode">
...
query MyQuery($title: String!, $name: String!) {
  seminar(where: {_or: [{course_group: {course: {title: {_similar: $title}}}}, {course_group: {faculty: {name: {_similar: $name}}}}]} ) {
    date
    start
    end
    id
