...
INPUT_FILEPATH <- "~/github/OpenClassroom/data/semester-1-fall-2020-formatted-20200708.csv"
# Declare some value constants. 
SEMESTER_CODE <- "AY2021-1"
TIMEZONE <- "+08:00"
DATE_FORMAT <- "%d-%m-%Y"
STARTING_SUNDAY_DATE <- as.Date("9-8-2020", DATE_FORMAT) # Sunday of week 1.
...
course_schedule <- validated %>% 
  ...
  rename(module_code=Module.Code,
         group_code=Section,
         day=Day,
         start=Start,
         end=End, 
...
SEMINAR_SCHEMA <- c("module_code", "group_code", "semester_code", "week", "date", "start", "end", "location_code", "area")
seminar <- data.frame(matrix(ncol=length(SEMINAR_SCHEMA), nrow=0, dimnames=list(NULL,SEMINAR_SCHEMA)))
...
tagged_as <- validated %>%
  ...
  mutate(tag_label=recode(tag_label,
...
################## Get location table. 
location <- read.csv("~/github/OpenClassroom/data/room-legend-formatted.csv", stringsAsFactors = FALSE, header=FALSE) %>% mutate(across(where(is.character), trimws))
